...
function showShare() {
  ...
  if (!shareContainer) {
    ...
    newShareContainer.innerHTML = `
      <p>My TCAL Personality Test result is **${userResult.archetype}** with a similarity score of ${userResult.similarityScore.toFixed(3)}. My secondary type is ${userResult.secondaryArchetype} (${userResult.secondarySimilarityScore.toFixed(3)}). https://llamawa.re/TCAL-Personality/</p>
    `;
...
const characterAnalyses = {
  ...
  "Mr. Graves": {
    ...
    additionalInfo: `You are <strong>Douglas Graves</strong>, the Peacemaker. You are devoted to the quest for internal and external peace for yourself and others. You work to maintain your peace of mind just as you work to establish peace and harmony in your world.<br><br>
...
function showAnalysis() {
  let analysisContainer = document.getElementById('analysis');
  const userResult = calculateResult();
  const data = characterAnalyses[userResult.archetype];
  if (!data) return;

  if (!analysisContainer) {
    analysisContainer = document.createElement('div');
    analysisContainer.id = 'analysis';
    document.getElementById('quiz-content').appendChild(analysisContainer);
  }
...
function showSecondary() {
  ...
  if (!secondaryContainer) {
    ...
    const secondaryData = characterAnalyses[userResult.secondaryArchetype];

    let analysisHTML = '';
    analysisHTML = `
      <p><strong>Motivation:</strong> ${secondaryData.motivation}</p>
      <p>${secondaryData.additionalInfo}</p>
      <p><strong>Recommendations:</strong> ${secondaryData.recommendations}</p>
    `;
